...
export default function DashboardPreview() {
  ...
  return (
    <div className="w-full bg-[#050816] text-white p-4 rounded-2xl border border-white/5 space-y-3 text-left font-sans text-xs select-none shadow-2xl relative overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2 border-b border-white/5 pb-2.5">
        ...
        <div className="flex gap-1.5 self-start sm:self-center">
          {resolvedCount > 0 && (
            <button
              onClick={handleReset}
              className="px-1.5 py-0.5 bg-white/5 hover:bg-white/10 text-white/70 hover:text-white border border-white/10 rounded-md text-[8px] font-bold transition duration-200 cursor-pointer"
            >
              🔄 Reset
            </button>
          )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
        <div className="bg-[#0B1220] border border-white/5 p-2 rounded-xl transition hover:border-indigo-500/20">
          <span className="text-slate-400 text-[8px] uppercase font-bold tracking-wider block">City Risk Index</span>
          <div className="flex items-baseline gap-1.5 mt-0.5">
            <span className="text-base font-black block text-indigo-400">{criScore}</span>
            {criScore > 80 ? (
              <span className="text-[7px] text-rose-400 font-extrabold uppercase bg-rose-500/10 px-1 py-0.2 rounded">Severe</span>
            ) : criScore > 50 ? (
              <span className="text-[7px] text-amber-400 font-extrabold uppercase bg-amber-500/10 px-1 py-0.2 rounded">Elevated</span>
            ) : (
              <span className="text-[7px] text-emerald-400 font-extrabold uppercase bg-emerald-500/10 px-1 py-0.2 rounded">Safe</span>
            )}
          </div>
        </div>

        <div className="bg-[#0B1220] border border-white/5 p-2 rounded-xl transition hover:border-indigo-500/20">
          <span className="text-slate-400 text-[8px] uppercase font-bold tracking-wider block">Critical Incidents</span>
          <span className="text-base font-black block text-rose-400 mt-0.5">{criticalCount}</span>
        </div>

        <div className="bg-[#0B1220] border border-white/5 p-2 rounded-xl transition hover:border-indigo-500/20">
          <span className="text-slate-400 text-[8px] uppercase font-bold tracking-wider block">High Risk Zones</span>
          <span className="text-base font-black block text-orange-400 mt-0.5">{highRiskCount}</span>
        </div>

        <div className="bg-[#0B1220] border border-white/5 p-2 rounded-xl transition hover:border-indigo-500/20">
          <span className="text-slate-400 text-[8px] uppercase font-bold tracking-wider block">Mitigated Reports</span>
          <span className="text-base font-black block text-emerald-400 mt-0.5">{resolvedCount}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
        ...
        <div className="lg:col-span-2 space-y-3">
          ...
          <div className="bg-[#0B1220] border border-white/5 rounded-xl p-2.5 space-y-1.5 relative overflow-hidden">
            <div className="flex justify-between items-center border-b border-white/5 pb-1.5">
              ...
              <span className="text-[7px] text-slate-500 uppercase tracking-widest font-mono">GPS Nodes</span>
            </div>

            <div className="h-[110px] w-full bg-[#050816] rounded-lg border border-white/5 relative flex items-center justify-center overflow-hidden">
              {/* Dot grid simulation */}
              <div className="absolute inset-0 opacity-[0.06] bg-[radial-gradient(#6366f1_1px,transparent_1px)] [background-size:12px_12px]" />
              ...
              {/* Grid Lines */}
              <svg className="w-full h-full opacity-20 absolute inset-0 p-2" viewBox="0 0 100 60" fill="none">
              ...
              {issues.map((item) => {
                ...
                return (
                  ...
                  >
                    {isPending ? (
                      <span className="flex h-3.5 w-3.5 -translate-x-1/2 -translate-y-1/2">
                        <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 bg-indigo-400`} />
                        <span className={`relative inline-flex rounded-full h-3.5 w-3.5 border border-white/20 items-center justify-center text-[6px] font-black ${
                      ...
                      <span className="flex h-3 w-3 -translate-x-1/2 -translate-y-1/2 animate-scaleIn">
                        <span className="relative inline-flex rounded-full h-3 w-3 bg-emerald-500/20 border border-emerald-500 text-emerald-400 items-center justify-center text-[6px] font-black">
                          ✓
                        </span>
                      </span>
                    )}
              ...
              {activeIssues.length > 0 && (
                <div className="absolute bottom-1.5 left-1.5 bg-black/85 border border-white/10 px-2 py-1 rounded-lg shadow-xl text-[8px] max-w-[130px] backdrop-blur-md animate-fadeIn">
                  <div className="font-bold text-white flex items-center gap-1">
                    <span className="h-1 w-1 rounded-full bg-red-500 animate-pulse" />
                    {activeIssues[0].location}
                  </div>
                  <div className="text-indigo-300 font-extrabold mt-0.5">
                    CRI: {activeIssues[0].risk}
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="bg-[#0B1220] border border-white/5 rounded-xl p-2.5 space-y-1.5">
            <div className="flex justify-between items-center border-b border-white/5 pb-1.5">
              <span className="font-extrabold text-white text-[10px] flex items-center gap-1">
                <span>⚡</span> Action Plan & Dispatch
              </span>
              <span className="text-[8px] text-indigo-400 font-bold uppercase tracking-wider">Priorities</span>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              {issues.map((item) => {
                ...
                if (item.type === "sewer") {
                  actionTitle = `Clear Sewer – ${item.location}`;
                  actionDesc = "Severe blockade. Dispatch vacuum extraction.";
                  ...
                  actionTitle = `Repair Pothole – ${item.location}`;
                  actionDesc = "High-traffic artery. Immediate cold asphalt mix.";
                  ...
                  actionTitle = `Clear Waste – ${item.location}`;
                  actionDesc = "Commercial dumping blocking right of way.";
                  ...
                  actionDesc = "Unsecured scaffolding structural audit.";
                ...
                return (
                  <div
                    key={item.id}
                    className={`p-2 border rounded-lg flex flex-col justify-between gap-1.5 text-[9px] transition duration-300 ${cardColor}`}
                  >
                    <div className="space-y-0.5">
                      ...
                      <p className="text-[8px] text-slate-400 leading-normal line-clamp-1">
                        {actionDesc}
                      </p>
                    </div>

                    <div className="flex justify-between items-center pt-1 border-t border-white/5 gap-1.5 mt-0.5">
                      <span className="text-[8px] font-extrabold text-emerald-400">
                        -{Math.round(item.risk * 0.15)} CRI
                      </span>
                      {isPending ? (
                        <button
                          onClick={() => handleDispatch(item.id)}
                          className="px-1.5 py-0.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded text-[8px] font-black uppercase tracking-wider transition duration-200 hover:scale-[1.03] active:scale-95 cursor-pointer shadow"
                        >
                          Dispatch
                        </button>
                      ) : (
                        <span className="text-[8px] text-emerald-400 font-black flex items-center gap-0.5 py-0.5">
                          ✓ Dispatched
                        </span>
                      )}
                    </div>
        ...
        <div className="space-y-3">
          ...
          {/* TOP RISK AREAS */}
          <div className="bg-[#0B1220] border border-white/5 rounded-xl p-2.5 space-y-2">
            <span className="font-extrabold text-white text-[10px] flex items-center gap-1 border-b border-white/5 pb-1.5">
              <span>🏙️</span> Risk Leaderboard
            </span>
            
            <div className="space-y-2">
              {issues.map((item) => {
                const isPending = item.status === "Pending";
                const displayRisk = isPending ? item.risk : 15;
              ...
              <table className="w-full text-left text-[8px]">
                <thead>
                  <tr className="text-slate-500 border-b border-white/5 font-bold">
                    ...
                    <th className="pb-0.5">Location</th>
                    <th className="pb-0.5 text-center">Time</th>
                  ...
                  {issues.map((item) => {
                    ...
                    return (
                      <tr key={item.id} className="transition duration-300">
                        <td className="py-1 font-bold capitalize">{item.type}</td>
                        <td>{item.location.split(" ")[0]}</td>
                        <td className={`text-center font-mono ${remainingColor}`}>{remaining}</td>
                        <td className={`text-right font-extrabold text-[7px] uppercase ${isPending ? (item.severity === "Critical" ? "text-rose-400 animate-pulse" : "text-orange-400") : "text-emerald-400"}`}>
                          {statusLabel}
                        </td>
                      </tr>
                    );
